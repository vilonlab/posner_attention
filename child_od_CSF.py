...
# Access the patched questplus files
project_dir = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, os.path.join(project_dir, "questplus_patched"))
from questplus.qp_patched import QuestPlus
...
# trial conditions for both, prac and exp trials
...
CATCH_TRIAL_TYPES = data.createFactorialTrialList({
            'orientation': ORIENTATIONS,
            'gabor_position': GABOR_POSITIONS,
            'freq_condition': FREQ_CONDITIONS,
...
def consecutive_check(trial_list):
    """ Only for practice blocks. Checks the trial list to ensure that there are no more than MAX_CONSECUTIVE_TRIALS consecutive trials with the same sf condition """
...
def create_trial_list(block_type):
    ...
    if block_type == 'experiment':
        ...
        handler_exp = data.TrialHandler(nReps=EXP_TRIAL_PRESENTATIONS, method='random', 
        ...
        trial_list = interleave_catch_trials(exp_trials, catch_trials)
        return trial_list
    else:
...
def run_trial(trial, practice = False, practice_contrasts = None, block_num = None):
    ...
    thisExp.addData('trial_type', trial['type'])
    ...
    # Set QP algorithm logic for experiment trials
        ...
        if trial['type'] == 'real':
            ...
            intensity = next_stim['contrast']
            frequency = next_stim['spatial_freq']
            gabor.contrast = intensity
            gabor.sf = frequency
    ...
    el_tracker.sendMessage('!V TRIAL_VAR trial_type %s' % trial['type'])
    ...
    if not practice and trial['type'] == 'real':
        thisExp.addData('qp_gmax', gmax)
        thisExp.addData('qp_fmax', fmax)
        thisExp.addData('qp_beta_bw', beta_bw)
        thisExp.addData('qp_lapse', lapse_rate)
        print(f"Next Intensity: {intensity}")
        print(f"Next Frequency: {frequency}")
...
def run_practice_block(block_num):
    ...
    if block_num == 1:
        while accuracy <= ACCURACY_THRESHOLD and repeat_count < MAX_PRACTICE_REPEATS:
            ...
            total_trials = len(PRAC_TRIAL_TYPES)*PRACT1_PRESENTATIONS
